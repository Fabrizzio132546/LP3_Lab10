...
// fondo animado area de juego con shader verde estilo balatro
public class FondoJuego extends JPanel {
    ...
    private float[] colVerdeBrillante = { 0.3f, 0.85f, 0.5f };
    private float[] colVerdeOscuro = { 0.1f, 0.45f, 0.25f };
    private float[] colNegro = { 0.05f, 0.15f, 0.1f };
    ...
    // shader principal efecto remolino verde sin centro
        ...
        IntStream.range(0, altoRender).parallel().forEach(py -> {
            for (int px = 0; px < anchoRender; px++) {
                ...
                if (val > lineThickness) {
                    // zona verde brillante
                    float light = smoothstep(lineThickness, 1.0f, val);
                    r = colVerdeBrillante[0] * light;
                    g = colVerdeBrillante[1] * light;
                    b = colVerdeBrillante[2] * light;
                } else if (val < -lineThickness) {
                    // zona verde oscuro
                    float light = smoothstep(lineThickness, 1.0f, Math.abs(val));
                    r = colVerdeOscuro[0] * light;
                    g = colVerdeOscuro[1] * light;
                    b = colVerdeOscuro[2] * light;
                } else {
