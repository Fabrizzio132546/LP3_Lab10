...
public class PanelItemColeccion extends JPanel {
    ...
    private void cargarImagen() {
        ...
        try {
            String rutaCompleta = "/javatro/recursos/imagenes" + info.getRutaImagen();

            System.out.println("🔍 COLECCIÓN intentando cargar: " + rutaCompleta);
            URL url = getClass().getResource(rutaCompleta);
            System.out.println("🔍 URL resultado: " + url);

            if (url != null) {
                imagen = javax.imageio.ImageIO.read(url);
            } else {
                System.err.println(" No se encontró la imagen en: " + rutaCompleta);
            }
        } catch (Exception e) {
